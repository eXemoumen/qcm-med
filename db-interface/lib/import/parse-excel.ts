...
function parseCorrectAnswers(raw: string): string[] {
  ...
  // Accepts: "A,C" or "AC" or "A, C" or "a,c"
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Find the Questions sheet (skip Instructions)
        const sheetName = workbook.SheetNames.find(
          (name) => name.toLowerCase().includes('question')
        ) || workbook.SheetNames[0];

        if (!sheetName) {
          reject(new Error('No sheet found in the Excel file'));
          return;
        }
        ...
        for (let i = 0; i < jsonData.length; i++) {
          ...
          const questionData: CreateQuestionData = {
            ...
            exam_type: String(mapped.exam_type || '').trim(),
            exam_year: parseInt(String(mapped.exam_year || '0'), 10) || 0,
            number: parseInt(String(mapped.number || '0'), 10) || 0,
            question_text: String(mapped.question_text || '').trim(),
            speciality: mapped.speciality ? String(mapped.speciality).trim() : undefined,
            cours: mapped.cours ? parseCours(String(mapped.cours)) : undefined,
